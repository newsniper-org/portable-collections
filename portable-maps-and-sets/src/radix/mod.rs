...
//!
//!   1. **persistent / on-disk backbone** — [`RadixOrderedMap`] (a naive
//!      byte-radix) and [`ArtOrderedMap`] (a path-compressed **Adaptive Radix
//!      Tree**). Both are `no_std` + zero-runtime-dependency: immutable nodes
//!      shared via `Arc`, every update path-copies the touched path, and a
...
//!      [`ShardedRadixOrderedMap`] and the seq-stamped [`ShardedArtOrderedMap`]
//!      give the same CoW structure a lock-free atomic-`Arc` root (`arc-swap`):
...
//! are generic `V: Clone` (CoW path-copy clones nodes, hence `Clone`). The two
//! single-root maps implement [`OrderedMap`] / [`SnapshotMap`] (this crate) and
//! [`Container`](portable_collection_primitives::Container).
//!
//! [`RadixOrderedMap`]: crate::radix::RadixOrderedMap
//! [`ArtOrderedMap`]: crate::radix::ArtOrderedMap
...
#[cfg(feature = "concurrent")]
pub use concurrent::{ShardedRadixOrderedMap, ShardedRadixSnapshot};
#[cfg(feature = "concurrent")]
pub use concurrent_art::{ShardedArtOrderedMap, ShardedArtSnapshot};
